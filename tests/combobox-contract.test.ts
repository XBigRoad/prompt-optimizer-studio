...
const comboboxPath = path.resolve(process.cwd(), 'src/components/shared/ui/model-alias-combobox.tsx')
const selectFieldPath = path.resolve(process.cwd(), 'src/components/shared/ui/select-field.tsx')
...
test('dropdown components stop wheel propagation at the popover layer to avoid scroll chaining into the page', () => {
  const comboboxSource = fs.readFileSync(comboboxPath, 'utf8')
  const selectSource = fs.readFileSync(selectFieldPath, 'utf8')
